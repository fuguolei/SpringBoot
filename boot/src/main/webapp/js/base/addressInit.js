...
$(function() {
      var provinceHtml = new Array();
      for (var i = 0; i < list1.length; i++) {
            var name = list1[i];
            provinceHtml.push("<option value='");
            provinceHtml.push(name);
            provinceHtml.push("'>");
            provinceHtml.push(name);
            provinceHtml.push("</option>");
      }
      var provinceStr = provinceHtml.join("");
      $(".province").html(provinceStr);

      var cityHtml = new Array();
      for (var i = 0; i <　list2[0].length; i++) {
            var name = list2[0][i];
            cityHtml.push("<option value='");
            cityHtml.push(name);
            cityHtml.push("'>");
            cityHtml.push(name);
            cityHtml.push("</option>");   
      }
      var cityStr = cityHtml.join(""); 
      $(".city").html(cityStr);
      ...
      $(".province").change(function() {
            ...
            var index = indexOf(province);
            var cityArray = list2[index];
            var $city = $(this).siblings(".city");
            var cityHtml = new Array();
            for (var i = 0; i < cityArray.length; i++) {
                  var name = cityArray[i];
                  console.log(name);
                  cityHtml.push("<option value='");
                  cityHtml.push(name);
                  cityHtml.push("'>");
