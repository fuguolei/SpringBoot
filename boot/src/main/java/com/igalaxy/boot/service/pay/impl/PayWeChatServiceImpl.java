...
public class PayWeChatServiceImpl implements PayWeChatService {
    ...
    public BaseResult unifiedOrder(Long orderId, String ip) {
        ...
        try {
            ...
            e.printStackTrace();
            return new BaseResult(false, "统一下单失败");
        }
        if (!"SUCCESS".equals(resp.get("return_code")))
            return new BaseResult(false, resp.get("return_msg"));
        ...
        try {
            ...
            e.printStackTrace();
            return new BaseResult(false, "签名失败，请重试");
        }
        return new BaseResult(true, jsPay);
    ...
    public BaseResult orderNotify(String requestBody) {
        ...
        try {
            ...
            if (body.get("return_code").equals("SUCCESS") && WXPayUtil.isSignatureValid(body, config.getKey())) {
                ...
                result = new BaseResult(true, "成功");
            } else {
                result = new BaseResult(false, "失败");
            }
        } catch (Exception e) {
            e.printStackTrace();
            result = new BaseResult(false, "异常");
        }
        return result;
    }
}
